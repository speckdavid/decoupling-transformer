...
void LeafStateSpace::build_leaf_state_space(FactorID leaf,
    ...
    while (change) {
        ...
        for (LeafStateHash id(0); id < state_registry->size(leaf); ++id){
            ...
            const LeafState curr_leaf_state = state_registry->get_leaf_state(id, leaf);
            closed[id] = true;

            assert(id == is_a_leaf_goal_state[leaf].size());
            if (check_is_goal_state(curr_leaf_state)) {
                is_a_leaf_goal_state[leaf].push_back(true);
                leaf_goal_states[leaf].push_back(id);
            } else {
                is_a_leaf_goal_state[leaf].push_back(false);
            }
            ...
            for (OperatorID op_id : applicable_ops){
                ...
                // this ensures that there is an entry leaf_state_predecessors[leaf][lid]
                // even in case succ_id has been generated by a center action
                if (succ_id >= leaf_state_predecessors[leaf].size()) {
                    leaf_state_predecessors[leaf].resize(succ_id + 1);
                }

                leaf_state_successors[leaf][id].emplace_back(op_id, succ_id);
                leaf_state_predecessors[leaf][succ_id].emplace_back(op_id, id);
            }
        }
    }
