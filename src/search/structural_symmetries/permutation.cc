...
void Permutation::print_cycle_notation() const {
    ...
    for (size_t i = 0; i < vars_affected.size(); i++)
        utils::g_log << from_vars[vars_affected[i]] << " -> " << vars_affected[i] << "  ";
    utils::g_log << endl;
...
void Permutation::replace_partial_state(vector<int> &state) const {
    for (size_t i = 0; i < affected_vars_cycles.size(); i++) {
        ...
        for (int j = affected_vars_cycles[i].size() - 1; j > 0; j--) {
            ...
            int from_val = state[from_var];
            if (from_val != -1) {
                auto [_, to_val] = get_new_var_val_by_old_var_val(from_var, from_val);
                state[to_var] = to_val;
            } else {
                state[to_var] = -1;
            }
        ...
        // writing the last one
        if (last_val != -1) {
            auto [to_var, to_val] = get_new_var_val_by_old_var_val(last_var, last_val);
            state[to_var] = to_val;
        } else {
