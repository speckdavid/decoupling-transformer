...
#    'F0.2s': 'lp(factoring_time_limit=30, strategy=mfa, add_cg_sccs=true, min_flexibility=0.2)',
    'Fs1L': 'lp(factoring_time_limit=30, strategy=mfa, add_cg_sccs=true)',
    'F0.2s1Mnopt': 'lp(factoring_time_limit=30, strategy=mfa, add_cg_sccs=true, min_flexibility=0.2, max_leaf_size=1000000, optimize_leaf_unique_lstate=false)',
...
h_predefine = ["--evaluator", "hff=ff(transform=adapt_costs(one))"]
search_option = "[hff], preferred=[hff]"

DRIVER_OPTS = ["--overall-memory-limit", "8G", "--overall-time-limit", "30m"]

CONFIGS.append(IssueConfig(f'ff-Fs1L', h_predefine + ['--root-task-transform', f"decoupled(factoring={factorings['Fs1L']})", '--search',  f'lazy_greedy({search_option})'], driver_options=DRIVER_OPTS))
CONFIGS.append(IssueConfig(f'ff-F0.2s1Mnopt', h_predefine + ['--root-task-transform', f"decoupled(factoring={factorings['F0.2s1Mnopt']}, conclusive_leaf_encoding=BASIC, skip_unnecessary_leaf_effects=FALSE)", '--search',  f'lazy_greedy({search_option})'], driver_options=DRIVER_OPTS))
