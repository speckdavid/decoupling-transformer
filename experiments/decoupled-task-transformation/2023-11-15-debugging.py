...
REVISION = "1c2d2af3fb0ec155b099cc26aa69fe0d3547d2d0"
REVISIONS = [REVISION]
...
for h_name, heuristic in heuristics.items():
    ...
    for dec_name, dec in factorings.items():
        CONFIGS.append(IssueConfig(f'{h_name}-{dec_name}', ['--root-task-transform', f"decoupled(factoring={dec})", '--search',  f'lazy_greedy([{heuristic}])'], driver_options=DRIVER_OPTS))
        CONFIGS.append(IssueConfig(f'{h_name}-{dec_name}-deb', ['--root-task-transform', f"decoupled(factoring={dec})", '--search',  f'lazy_greedy([{heuristic}])'], build_options=['debug'], driver_options=DRIVER_OPTS + ['--build', 'debug']))
